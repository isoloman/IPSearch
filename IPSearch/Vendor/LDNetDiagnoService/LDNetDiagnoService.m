...
//    [self recordCurrentAppVersion];
    ...
    //未联网不进行任何检测
    if ([_curNetType isEqualToString:@"无网络"]) {
        _isRunning = NO;
        [self recordStepInfo:@"\n当前主机未联网，请检查网络！"];
        [self recordStepInfo:@"\n网络诊断结束\n"];
...
//        [self recordOutIPInfo];
    ...
    if (_isRunning) {
        ...
        if (_isRunning&&_type == 0) {
            [self pingDialogsis:_hostAddress.count==0];
        }
    }


    if (_isRunning&&_type == 1) {
        //开始诊断traceRoute
        NSString * msg = [NSString stringWithFormat:@"\n通过最多50个跃点跟踪 \n\n到%@[%@]的路由:\n\n",_dormain,_dormain];
        [self recordStepInfo:msg];
...
//    [self recordStepInfo:[NSString stringWithFormat:@"\n\n诊断域名 %@...\n", _dormain]];
    //判断是否联网以及获取网络类型
//    NSArray *typeArr = [NSArray arrayWithObjects:@"2G", @"3G", @"4G", @"5G", @"WIFI",@"WWAN", nil];
    _curNetType = [YCProjectManager getNetType];
    if ([_curNetType isEqualToString:@"无网络"]) {
...
//    [self recordStepInfo:[NSString stringWithFormat:@"当前本机IP: %@", _localIp]];
    ...
    if ([[_curNetType lowercaseString] isEqualToString:@"wifi"]) {
...
- (void)pingDialogsis:(BOOL)pingLocal
{
    ...
    if (pingLocal) {
        [pingInfo addObject:@"DNS解析结果: 解析失败"];
...
//    }
    ...
    if ([_hostAddress count] > 0) {
        ...
        NSString * msg = [NSString stringWithFormat:@"%@",_hostAddress.firstObject];
        [pingInfo addObject:msg];
    }

    //不管服务器解析DNS是否可达，均需要ping指定ip地址
    if([_localIp rangeOfString:@":"].location == NSNotFound&&kPingOpenServerIP.length){
...
- (void)netPingDidEnd
{
    // net
    _isRunning = NO;
    float total = _netPinger.sucTime+_netPinger.failTime;
    NSString * msg = [NSString stringWithFormat:@"\n已发送=%.0f已接收=%ld丢失=%ld(%.1f%%丢失)\n",total,(long)_netPinger.sucTime,(long)_netPinger.failTime,isnan(100*_netPinger.failTime/total)?0.0:100*_netPinger.failTime/total];
    [self recordStepInfo:msg];
    
    if (_netPinger.failTime != total) {
        NSString * timeMsg = [NSString stringWithFormat:@"\n最短=%ldms,最长=%ldms,平均=%.0fms\n",_netPinger.minTime,_netPinger.maxTime,_netPinger.totalTime/MAXCOUNT_PING];
        [self recordStepInfo:timeMsg];
    }
