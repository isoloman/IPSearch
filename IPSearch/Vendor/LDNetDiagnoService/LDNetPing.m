...
- (void)runWithHostName:(NSString *)hostName normalPing:(BOOL)normalPing{
    ...
    } while (_pinger != nil || _sendCount <= MAXCOUNT_PING);
}
...
- (void)pingTimeout:(NSTimer *)index
{
    if ([[index userInfo] intValue] == _sendCount && _sendCount <= MAXCOUNT_PING + 1 &&
        _sendCount > 1) {
        NSString *timeoutLog =
            [NSString stringWithFormat:@"ping: cannot resolve %@: 请求超时", _hostAddress];
        _failTime++;
...
#pragma unused(packet)
    ...
    long starttime = [LDNetTimer computeDurationSince:_startTime]/ 1000;
    ...
    NSString *successLog = [NSString
        stringWithFormat:@"%lu bytes from %@ icmp_seq=#%u type=%@ time=%ldms",
                         ...
                         icmpReplyType,
                         starttime];
    if (_maxTime == _minTime&& _maxTime == 0) {
        _maxTime = _minTime = starttime;
    }
    _maxTime = _maxTime >= starttime ? _maxTime: starttime;
    _minTime = _minTime <= starttime ? _minTime: starttime;
    _totalTime += starttime;
