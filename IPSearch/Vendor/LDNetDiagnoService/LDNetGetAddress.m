...
+ (NETWORK_TYPE)getNetworkTypeFromStatusBar {
    __block NETWORK_TYPE returnValue = NETWORK_TYPE_NONE;
    if (![[NSThread currentThread] isMainThread]) {
        dispatch_sync(dispatch_get_main_queue(), ^{
            returnValue = [self getNetworkTypeFromStatusBar];
        });
        return returnValue;
    }
    id _statusBar = nil;
            ...
            if (_wifiEntry && [[_wifiEntry valueForKeyPath:@"isEnabled"] boolValue]) {
                ...
                if (type) {
                    switch (type.integerValue) {
                        case 5:
                            returnValue = NETWORK_TYPE_4G;
                            break;
                        case 4:
                            returnValue = NETWORK_TYPE_3G;
                            break;
                            //                        case 1: // Return 1 when 1G.
                            //                            break;
                        case 0:
                            // Return 0 when no sim card.
                            returnValue = NETWORK_TYPE_NONE;
                        default:
                            returnValue = NETWORK_TYPE_WWAN;
                            break;
                    }
                }
            }
